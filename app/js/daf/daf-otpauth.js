...
(function () {
    ...
    $document.on('otpauthtotpsetup_calculate.app', function (e) {
        ...
        if (clientAppIsTrusted() && accountManagerIsEnabled()) {
            pleaseWait()
    ...
    function postOAuth2Consent(consent) {
        _app.restful({
            ...
            $app.touch.notify({ text: error.errors[0].reason + ': ' + error.errors[0].message, force: true, duration: 'long' });
        });
    }
        ...
        if (!clientAppIsTrusted())
            scopeList.forEach(function (scope) {
                if (scope && scope != 'openid') {
                    ...
                    if (!scopeDef)
                        scopeDef = { hint: scope, description: String.format('This scope is not known to {0} application.', $app.touch.appName()) };
                    if (!scopeDef.icon)

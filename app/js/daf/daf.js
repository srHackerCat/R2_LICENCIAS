...
(function () {
    ...
    _app.cookie = function (name, value, days, hours, minutes) {
        ...
        document.cookie = name + "=" + value + "; SameSite=Strict" + expires + "; path=" + __baseUrl;
    };
    ...
    _app.restful = function (options) {
        return new Promise(function (resolve, reject) {
            _app.getScript('~/v2/js/restful-2.0.1', {
                then: function () {
    ...
    _app.prototype = {
        ...
        canSync: function () {
            var that = this,
                ...
                externalFilterFirstValue = master ? that._externalFilter[0].Value : null;
            return !that._busy() && (!master || selectedKey.length && selectedKey[0] != null && externalFilterFirstValue && externalFilterFirstValue != '%js%');
        },
        ...
        _onMethodFailed: function (err, response, context) {
            ...
            _touch.notify({ text: String.format('{0}\n\n{1}/{2} error {3}:\n\n{4}', err.get_message(), that._controller, that._viewId, err.get_exceptionType(), err.get_stackTrace()), duration: 'long', force: true });
        },

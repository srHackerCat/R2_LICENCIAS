...
namespace MyCompany.Services
{
    ...
    public partial class FacebookOAuthHandlerBase : OAuthHandler
    {
        ...
        {
            return string.Format("https://www.facebook.com/{0}/dialog/oauth?response_type=code&client_id={1}&redirect_uri={2}&scope={3}&state={4}", GetVersion(), Config.ClientId, Config.RedirectUri, Uri.EscapeDataString(Scope), Uri.EscapeDataString(GetState()));
        }

        protected override WebRequest GetAccessTokenRequest(string code, bool refresh)
        {
            return WebRequest.Create(string.Format("https://graph.facebook.com/{0}/oauth/access_token?client_id={1}&redirect_uri={2}&client_secret={3}&code={4}", GetVersion(), Config.ClientId, Config.RedirectUri, Config.ClientSecret, code));
        }
    ...
    public partial class GoogleOAuthHandlerBase : OAuthHandler
    {
        ...
        public override string GetAuthorizationUrl()
        {
            ...
            if (StoreToken)
                accessType = "offline";
            var authUrl = string.Format("https://accounts.google.com/o/oauth2/v2/auth?response_type=code&client_id={0}&redirect_uri={1}&scope={2}&state={3}&access_type={4}&prompt=select_account", Config.ClientId, Uri.EscapeDataString(Config.RedirectUri), Uri.EscapeDataString(Scope), Uri.EscapeDataString(GetState()), accessType);
            return authUrl;
        }
        ...
        protected override WebRequest GetAccessTokenRequest(string code, bool refresh)
        {
            ...
            var body = string.Format("{0}={1}&client_id={2}&client_secret={3}&redirect_uri={4}&grant_type=authorization_code", codeType, code, Config.ClientId, Config.ClientSecret, Config.RedirectUri);
    ...
    public partial class LinkedInOAuthHandlerBase : OAuthHandler
    {
        ...
        public override string GetAuthorizationUrl()
        {
            return string.Format("https://www.linkedin.com/oauth/v2/authorization?response_type=code&client_id={0}&redirect_uri={1}&state={2}&scope={3}", Config.ClientId, Config.RedirectUri, Uri.EscapeDataString(GetState()), Uri.EscapeDataString(Scope));
        }
    ...
    public class SharePointOAuthHandlerBase : OAuthHandler
    {
        ...
        public override string GetAuthorizationUrl()
        {
            ...
            if (string.IsNullOrEmpty(version))
                version = "15";
            var authUrl = string.Format("{0}/_layouts/{1}/OAuthAuthorize.aspx?client_id={2}&scope={3}&response_type=code&redirect_uri={4}&state={5}", ClientUri, version, Config.ClientId, Uri.EscapeDataString(Scope), Uri.EscapeDataString(Config.RedirectUri), Uri.EscapeDataString(GetState()));
            return authUrl;
        }
        ...
        protected override WebRequest GetAccessTokenRequest(string code, bool refresh)
        {
            ...
            var body = string.Format("grant_type={0}&client_id={1}%40{2}&client_secret={3}&{4}={5}&redirect_uri={6}&resource=00000003-0000-0ff1-ce00-000000000000%2F{7}%40{2}", grantType, Config.ClientId, _realm, Uri.EscapeDataString(Config.ClientSecret), codeName, code, Config.RedirectUri, ClientUri.Substring(8));
            var bodyBytes = Encoding.UTF8.GetBytes(body);
            request.ContentType = "application/x-www-form-urlencoded";
            request.ContentLength = bodyBytes.Length;
            using (var stream = request.GetRequestStream())
    ...
    public partial class WindowsLiveOAuthHandlerBase : OAuthHandler
    {
        ...
        public override string GetAuthorizationUrl()
        {
            return string.Format("https://login.live.com/oauth20_authorize.srf?client_id={0}&scope={1}&response_type=code&redirect_uri={2}&state={3}", Config.ClientId, Uri.EscapeDataString(Scope), Uri.EscapeDataString(Config.RedirectUri), Uri.EscapeDataString(GetState()));
        }
    ...
    public partial class IdentityServerOAuthHandlerBase : OAuthHandler
    {
        ...
        public override string GetAuthorizationUrl()
        {
            return string.Format("{0}/connect/authorize?response_type=code&client_id={1}&redirect_uri={2}&scope={3}&state={4}", ClientUri, Config.ClientId, Config.RedirectUri, Uri.EscapeDataString(Scope), Uri.EscapeDataString(GetState()));
        }

...
namespace MyCompany.Services
{
    ...
    public partial class CloudIdentityOAuthHandlerBase : OAuthHandler
    {
        ...
        {
            return string.Format("{0}/oauth/auth?response_type=code&client_id={1}&redirect_uri={2}&scope={3}&state={4}", ClientUri, Config.ClientId, Uri.EscapeDataString(Config.RedirectUri), Uri.EscapeDataString(Scope), Uri.EscapeDataString(GetState()));
        }
    ...
    public partial class DnnOAuthHandlerBase : OAuthHandler
    {
        ...
        protected override WebRequest GetAccessTokenRequest(string code, bool refresh)
        {
            ...
            if (refresh)
                codeType = "access_token";
            var body = string.Format("{0}={1}&client_id={2}&client_secret={3}&redirect_uri={4}&grant_type=authorization_code", codeType, code, Config.ClientId, Config.ClientSecret, Uri.EscapeDataString(Config.RedirectUri));
            var bodyBytes = Encoding.UTF8.GetBytes(body);
            request.ContentType = "application/x-www-form-urlencoded";
            request.ContentLength = bodyBytes.Length;
            using (var stream = request.GetRequestStream())
